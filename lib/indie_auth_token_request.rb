class IndieAuthTokenRequest
  ...
  def initialize(params)
    if params[:grant_type] and params[:grant_type]=="authorization_code"
      ...
      raise "request must have a grant_type parameter of 'authorization_code'"
    end
    if params[:code]
      @code = params[:code]
    else
      raise "request must have a code"
    end
    ...
    if params[:code_verifier]
      ...
      raise "request must have a code_verifier"
    end      
  end  
end
